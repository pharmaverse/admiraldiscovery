# Contributing to admiraldiscovery

This outlines how to propose a change to admiraldiscovery. For a
...
2.  Run `usethis::use_version()` to bump the version to a release
    version (i.e. removing the `*.9xxx` suffix).
3.  Once these changes are merged into the `main` branch, run
    `usethis::use_github_release()` from the `main` branch.

After the release has been made, check the website was correctly
published and run `usethis::use_dev_version()` to bump the version
number to a development version number.

## Updating Variable Listing

If an update is made to the variable listing
(`inst/adrmial-lookup-book.csv`), run the (`data-raw/DATASET.R`) script
to add the updates to the internal dataset.
...
- Fork the package and clone onto your computer. If you haven’t done
  ...
  `usethis::create_from_github("pharmaverse/admiraldiscovery", fork = TRUE)`.
...
Please note that the admiraldiscovery project is released with a
[Contributor Code of
Conduct](https://pharmaverse.github.io/admiraldiscovery/dev/CODE_OF_CONDUCT.md).
