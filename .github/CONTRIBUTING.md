...
This outlines how to propose a change to admiraldiscovery.
...
1. Run `usethis::use_version()` to bump the version to a release version (i.e. removing the `*.9xxx` suffix).
1. Once these changes are merged into the `main` branch, run `usethis::use_github_release()` from the `main` branch.
